...
// =============================================================================
// resolveAuthFromQueue — minimax provider
// =============================================================================

describe('resolveAuthFromQueue — minimax provider', () => {
  const minimaxAccount = {
    id: 'acc-minimax',
    provider: 'minimax' as const,
    authType: 'api-key' as const,
    apiKey: 'eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.fake-minimax-key',
    isActive: true,
  ...
  it('skips minimax account when no model equivalence exists for the requested model', async () => {
    // Without an explicit equivalence, the resolver falls through to the
    // cross-provider check (line 401-402): detectProviderFromModel('claude-opus-4-6')
    // returns 'anthropic', supportedProvider is 'minimax', so the account is skipped.
    mockResolveModelEquivalent.mockReturnValueOnce(null);

    const result = await resolveAuthFromQueue('claude-opus-4-6', [minimaxAccount]);

    expect(result).toBeNull();
  });

  it('selects minimax account first when it precedes an anthropic account in the queue', async () => {
  ...
  it('skips unavailable minimax accounts', async () => {
    mockScoreProviderAccount.mockReturnValueOnce({ available: false, score: 0 });
  ...
  it('preserves the account baseURL when resolving the minimax account', async () => {
    ...
    // baseURL is preserved by resolveCredentialsForAccount (resolver.ts:537-548)
    expect(result?.baseURL).toBe('https://api.minimaxi.com/anthropic');
  });
});
